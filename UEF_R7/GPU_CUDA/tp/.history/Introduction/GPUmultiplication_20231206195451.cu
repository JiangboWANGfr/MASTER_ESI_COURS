 ...
 *          1. we calculate the row and column indices of each thread, done using the block index (blockIdx.x and blockIdx.y), block dimension (blockDim.x and blockDim.y), and thread index within the block (threadIdx.x and threadIdx.y). 
 * 
