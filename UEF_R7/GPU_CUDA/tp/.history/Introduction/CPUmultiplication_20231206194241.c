...
*  @details  we using 3 for loops to perform matrix multiplication
 *           1. the first for loop is to go through the rows of matrix A
 *           2. the second for loop is to go through the columns of matrix B
 *           3. the third for loop is to go through the columns of matrix A and rows of matrix B to
 *              perform the multiplication the index is the index of the element of matrix C ,
 *              which is calculated by the row and column

* @example . for example, to compute the element at row 1, column 1 of the result matrix, multiply every element in the first row of the first matrix by every element in the first column of the second matrix, and then take the sum of all those products. This will give you the value for the element at row 1, column 1 of the result matrix.
