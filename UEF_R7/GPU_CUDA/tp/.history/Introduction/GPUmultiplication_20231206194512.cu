 ...
 * @details in the GPU, we use the grid of blocks and threads to compute the multiplication of two matrices ,so we just need to use one for loop.
 *         the number of blocks is ceil((float)(cols)) / blockSize.x * ceil((float)(rows)) / blockSize.y
 * 
