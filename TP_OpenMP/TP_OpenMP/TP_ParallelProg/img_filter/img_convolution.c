...
#include <omp.h>
/**
 * @brief This function performs the convolution operation on the image in a parallel way
 * 
...
#           pragma omp atomic
    ...
    printf("Convolution time for parallel version : %f\n",  (double)(end - start));
}
/**
 * @brief This function performs the convolution operation on the image in a sequential way 
 * @note This code is not optimized for parallel execution and the sequential execution is faster (0.015 seconds) than the parallel execution (0.038 seconds)
 *
 * @param img : the image to be processed
 * @param coefficient  : the coefficient of the kernel
 */
void convolution_seq(image_t *img, float coefficient)
{
    double start = omp_get_wtime();
    pixel_t(*bitmap_in)[img->height] = (pixel_t(*)[img->height])img->data_in,
    (*bitmap_out)[img->height] = (pixel_t(*)[img->height])img->data_out;
