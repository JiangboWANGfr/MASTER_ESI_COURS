 ...
 *             These calculations ensure that each thread works on a unique element in the 
 *             resultant matrix matC
 *         2. the loop effectively performs the dot product of a row from matA and a column from matB.
 *         3. Each thread calculates one element of the resulting matrix matC. The index row * cols + col 
 *            identifies the position in matC where the result of this dot product is stored.
 * 
 * @note  the number of threads per block is 16*16 = 256, the number of blocks is 1024*1024/256 = 4096 
 *        the index x of each block is 0,1,2,...,63 and the index y of each block is 0,1,2,...,63
 *        and the index of each thread is 0,1,2,...,255
 * 
