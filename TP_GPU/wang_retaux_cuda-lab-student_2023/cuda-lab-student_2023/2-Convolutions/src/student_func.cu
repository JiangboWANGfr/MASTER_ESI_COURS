...
{
    // Calculate the global 2D position of this thread
    ...
    // Initialize the variables used for the blur effect
    float blur_value = 0.f;
    int filterHalf = filterWidth / 2;

    // Iterate over the filter array to apply the Gaussian blur
    for (int filter_r = -filterHalf; filter_r <= filterHalf; ++filter_r) {
        for (int filter_c = -filterHalf; filter_c <= filterHalf; ++filter_c) {
            ...
            // Get the value at (image_r, image_c)
            float image_value = static_cast<float>(inputChannel[image_r * numCols + image_c]);

            // Get the filter value
            float filter_value = filter[(filter_r + filterHalf) * filterWidth + filter_c + filterHalf];

            // Accumulate the blur value
            blur_value += image_value * filter_value;
        }
    }

    // Write our new pixel value out
    outputChannel[thread_2D_pos.y * numCols + thread_2D_pos.x] = static_cast<unsigned char>(blur_value);
}
   ...
   // Calculate the global 2D position of this thread
    ...
    for (int filter_r = -filterHalf; filter_r <= filterHalf; ++filter_r) {
        for (int filter_c = -filterHalf; filter_c <= filterHalf; ++filter_c) {
            ...
            // Get the filter value
            float filter_value = filter_constant[(filter_r + filterHalf) * filterWidth + filter_c + filterHalf];
        ...
        unsigned char* const blueChannel)
{
    ...
    int index = idx_y * numCols + idx_x;

     if (index >= numCols*numRows )
     {
         return;
     }
    //Calculate the linear index from the 2D indices

    // Check if the index is within the image bounds
    if (idx_x < numCols && idx_y < numRows) {
        // Separate the channels
        uchar4 rgba = inputImageRGBA[index];
        redChannel[index] = rgba.x;   // Red channel
        greenChannel[index] = rgba.y; // Green channel
        blueChannel[index] = rgba.z;  // Blue channel
    }
}
        ...
        const float* const h_filter, const size_t filterWidth)
{
    ...
    checkCudaErrors(cudaMemcpyToSymbol(filter_constant, h_filter, sizeof(float) * filterWidth * filterWidth));
}
        ...
        const int filterWidth)
{
    ...
    // this is the kernel for filtering the red channel
    gaussian_blur_simple<<<gridSize, blockSize>>>(d_red,
            d_redBlurred,
            numRows,
            numCols,
            d_filter,
            filterWidth);
    // this is the kernel for filtering the green channel
    gaussian_blur_simple<<<gridSize, blockSize>>>(d_green,
            d_greenBlurred,
            numRows,
            numCols,
            d_filter,
            filterWidth);
    // this is the kernel for filtering the blue channel
    gaussian_blur_simple<<<gridSize, blockSize>>>(d_blue,
            d_blueBlurred,
            numRows,
            numCols,
            d_filter,
        ...
        const int filterWidth)
{
    ...
    gaussian_blur_constant_memory<<<gridSize, blockSize>>>(d_red,
            d_redBlurred,
            numRows,
            numCols,
            filterWidth);
    gaussian_blur_constant_memory<<<gridSize, blockSize>>>(d_green,
            d_greenBlurred,
            numRows,
            numCols,
            filterWidth);
    gaussian_blur_constant_memory<<<gridSize, blockSize>>>(d_blue,
            d_blueBlurred,
            numRows,
            numCols,
            filterWidth);
        ...
        const int filterWidth)
{
   ...
   //TODO15: Set reasonable block size (i.e., number of threads per block)
    dim3 blockSize(filterWidth, filterWidth);
    printf("blockSize: %d, %d\n", blockSize.x, blockSize.y);
    ...
    // __shared__ float shared_filter[FILTER_WIDTH * FILTER_WIDTH];
    ...
    gaussian_blur_shared_memory<<<gridSize, blockSize>>>(d_red,
            d_redBlurred,
            numRows,
            numCols,
            filterWidth);
    gaussian_blur_shared_memory<<<gridSize, blockSize>>>(d_green,
            d_greenBlurred,
            numRows,
            numCols,
            filterWidth);
    gaussian_blur_shared_memory<<<gridSize, blockSize>>>(d_blue,
            d_blueBlurred,
            numRows,
            numCols,
            filterWidth);
